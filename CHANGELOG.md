...
- `backlinks-api.json` (Moz / Bing API keys, created by hand) was read without
  any permission hardening, so it usually stayed at the umask default
  (world-readable on POSIX) or the directory's inherited ACL on Windows. It is
  now restricted to the current user on every load, the same remediation the
  OAuth token gets. (#290)
